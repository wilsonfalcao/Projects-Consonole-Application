...
public class Criptografia
{
    ...
    {
        string cryptoKey = "d2lsc29uZmFsY2Fv";// Key_Get_Cripty();
        try
        {
    ...
    {
        string cryptoKey = Key_Get_Decripty();
        try
        {
            // Se a string não está vazia, executa a criptografia           
            if (!string.IsNullOrEmpty(text))
    ...
    private string Key_Get_Cripty()
    {
        DateTime xx = DateTime.Now;
        string seconds = xx.AddSeconds(0).Second.ToString();
        var B_String_Convert_Base = System.Text.Encoding.UTF8.GetBytes("wilsonfalcao");
        return Convert.ToBase64String(B_String_Convert_Base);
    }
    private string Key_Get_Decripty()
    {
